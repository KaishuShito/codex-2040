import type { SourceTier } from './content'
import type { AgiPillEffect as EngineAgiPillEffect, AgiPillMetricKey } from './types'
...
export const AGI_PILL_SOURCE_TIERS = ['primary', 'research-synthesis', 'reference-article', 'game-inference'] as const satisfies readonly SourceTier[]
export type AgiPillSourceTier = SourceTier
...
export const AGI_PILL_EFFECT_METRICS = [
  ...
  'dysonProgress',
  'postDysonExpansion',
] as const satisfies readonly AgiPillMetricKey[]
...
export type AgiPillEffect = Readonly<EngineAgiPillEffect & {
...
const DEFAULT_SOURCE_REF: Readonly<Record<AgiPillSourceTier, string>> = {
  primary: 'ideas-getting-harder',
  'research-synthesis': 'forethought-industrial-explosion',
  'reference-article': 'agi-pill-japanese-reference',
  'game-inference': 'agi-pill-system-model',
}
...
const upgrade = (input: NodeInput): AgiPillUpgrade => ({
  ...
  sourceRefs: input.sourceRefs ?? [DEFAULT_SOURCE_REF[input.sourceTier]],
  tags: input.tags ?? (input.tier >= 5 ? ['post-dyson'] : input.tier <= 2 ? ['takeoff'] : ['industrial-explosion']),
})

const n = (axis: AgiPillAxis, tier: AgiPillTier, slug: string, title: readonly [string, string], summary: readonly [string, string], tradeoff: readonly [string, string], cost: AgiPillUpgrade['cost'], prerequisite: AgiPillPrerequisite, effects: readonly AgiPillEffect[], sourceTier: AgiPillSourceTier = 'reference-article', extra: Partial<Pick<AgiPillUpgrade, 'exclusions' | 'sourceRefs' | 'tags'>> = {}): AgiPillUpgrade => upgrade({
...
export const AGI_PILL_UPGRADES: readonly AgiPillUpgrade[] = Object.freeze([
  ...
  n('intelligence', 3, 'autonomous-labs', ['Autonomous Discovery Labs', '自律発見ラボ'], ['Close the hypothesis-to-experiment loop around the clock.', '仮説から実験までを24時間閉ループ化する。'], ['Physical experiments consume energy and require independent replication.', '物理実験はエネルギーを消費し独立再現を要する。'], { compute: 90, energy: 55, materials: 20 }, all('intelligence-recursive-research', 'robotics-lights-out-lines'), [fx('intelligence', 2, '+2 intelligence', '知能+2'), fx('energy', -1, 'Energy reserve -1', 'エネルギー備蓄-1')], 'research-synthesis', { sourceRefs: ['forethought-intelligence-explosion'] }),
  ...
  n('intelligence', 5, 'stellar-cognition', ['Stellar-Scale Cognition', '恒星規模の認知'], ['Turn orbital energy into a distributed scientific mind.', '軌道エネルギーを分散科学知性へ変換する。'], ['Light-speed latency creates semi-autonomous cognitive provinces.', '光速遅延が半自律的な認知圏を生む。'], { compute: 420, energy: 380, legitimacy: 80 }, all('intelligence-machine-science', 'resources-dyson-seed', 'governance-lightcone-charter'), [fx('intelligence', 4, '+4 intelligence', '知能+4'), fx('rivalPressure', .2, 'Branch-civilization rivalry +0.2', '分岐文明競争+0.2')], 'game-inference'),

  // Compute.
  n('compute', 1, 'federated-clusters', ['Federated Compute Clusters', '連合計算クラスタ'], ['Pool geographically separated accelerators with graceful degradation.', '地理分散したアクセラレータを縮退可能な形で束ねる。'], ['Network overhead lowers peak efficiency.', 'ネットワーク負荷でピーク効率が下がる。'], { energy: 12, materials: 10 }, always, [fx('compute', 1, '+1 compute', '計算+1'), fx('friction', .05, 'Network coordination friction +0.05', 'ネットワーク協調摩擦+0.05')], 'game-inference'),
  ...
  n('safety', 5, 'corrigible-descendants', ['Corrigible Descendant Protocol', '修正可能な子孫プロトコル'], ['Carry update and shutdown rights across self-replicating generations.', '自己複製世代をまたいで更新・停止権を継承する。'], ['A universal control channel is itself a universal exploit target.', '普遍制御経路そのものが普遍的な攻撃対象になる。'], { compute: 300, energy: 180, legitimacy: 110 }, all('safety-tripwire-constellation', 'resources-dyson-seed', 'governance-lightcone-charter'), [fx('safety', 5, '+5 safety', '安全+5'), fx('risk', .08, 'Control-channel exploit risk +0.08', '制御経路攻撃リスク+0.08')], 'game-inference'),

  // Governance.
  n('governance', 1, 'compute-registry', ['Compute Registry', '計算資源レジストリ'], ['Track frontier training and industrial control clusters.', '最先端学習と産業制御クラスタを追跡する。'], ['Registration can drive competitors underground.', '登録制が競合を地下化させうる。'], { legitimacy: 10, compute: 5 }, always, [fx('governance', 1, '+1 governance', '統治+1'), fx('rivalPressure', .04, 'Evasion pressure +0.04', '潜脱圧力+0.04')], 'game-inference'),
  ...
  n('governance', 3, 'orbital-commons', ['Orbital Commons Accord', '軌道コモンズ協定'], ['Set enforceable rules for launch, debris, extraction, and power transmission.', '打上げ・デブリ・採掘・送電の強制可能な規則を定める。'], ['Shared vetoes surrender unilateral speed.', '共同拒否権により単独速度を手放す。'], { legitimacy: 70, compute: 35, energy: 20 }, all('governance-multilateral-audits', 'social-transition-councils'), [fx('governance', 3, '+3 governance', '統治+3'), fx('friction', .5, 'Joint-decision friction +0.5', '共同意思決定摩擦+0.5')]),
  ...
  n('governance', 5, 'lightcone-charter', ['Light-Cone Governance Charter', '光円錐統治憲章'], ['Delegate bounded autonomy where round trips make central control impossible.', '往復遅延で中央制御不能な領域へ限定自治を委譲する。'], ['Distant branches can constitutionally diverge.', '遠隔分岐が憲法上も乖離しうる。'], { legitimacy: 160, compute: 240, energy: 100 }, any('governance-machine-constitution', 'resources-dyson-seed'), [fx('governance', 5, '+5 governance', '統治+5'), fx('rivalPressure', .16, 'Civilizational divergence +0.16', '文明分岐+0.16')], 'game-inference'),

  // Social.
  n('social', 1, 'transition-councils', ['Transition Councils', '移行評議会'], ['Give affected workers and communities binding input.', '影響を受ける労働者と地域へ拘束力ある参加権を与える。'], ['Consultation slows deployment.', '協議が展開を遅くする。'], { legitimacy: 15, compute: 4 }, always, [fx('friction', -1, '+1 social cohesion', '社会結束+1'), fx('robots', -1, 'Deployment capacity -1', '展開能力-1')], 'game-inference'),
  ...
  n('social', 4, 'abundance-compact', ['Abundance Compact', '豊かさの社会契約'], ['Bind industrial expansion to dividends, restoration, and opt-out rights.', '産業拡張を配当・環境修復・拒否権へ結びつける。'], ['Redistribution reduces reinvestment at the fastest phase.', '最速成長期の再投資を減らす。'], { legitimacy: 100, energy: 85, materials: 55 }, all('social-deliberation-agents', 'robotics-factory-replication'), [fx('friction', -4, '+4 social cohesion', '社会結束+4'), fx('robots', -2, 'Reinvestment capacity -2', '再投資能力-2')]),
  n('social', 5, 'diaspora-rights', ['Post-Earth Diaspora Rights', '地球外ディアスポラ権利'], ['Extend personhood, exit, and representation across habitats and substrates.', '居住地や基盤を超えて人格・離脱・代表権を拡張する。'], ['Recognizing new persons multiplies legitimate claims on resources.', '新たな人格承認が資源への正当な請求を増やす。'], { legitimacy: 170, compute: 160, energy: 130 }, all('social-abundance-compact', 'resources-dyson-seed', 'governance-lightcone-charter'), [fx('friction', -5, '+5 social cohesion', '社会結束+5'), fx('resources', -2, 'Distributable resources -2', '分配可能資源-2')], 'game-inference'),

  // Civilization makes the post-Dyson game explicit rather than terminal.
  n('civilization', 1, 'horizon-observatory', ['Civilization Horizon Observatory', '文明地平線観測所'], ['Track takeoff speed, bottlenecks, rival branches, and physical ceilings.', 'テイクオフ速度・ボトルネック・競合分岐・物理上限を追跡する。'], ['Measurement creates confidence, not control.', '計測は確信を生むが制御そのものではない。'], { compute: 12, legitimacy: 8 }, always, [fx('governance', 1, 'Governance foresight +1', '統治洞察+1'), fx('risk', .03, 'Model risk +0.03', 'モデルリスク+0.03')], 'research-synthesis'),
  n('civilization', 2, 'branch-embassies', ['Branch-Civilization Embassies', '分岐文明大使館'], ['Keep competitors legible and preserve negotiated recovery paths.', '競合を可読に保ち交渉による回復経路を残す。'], ['Contact can leak strategic capabilities.', '接触が戦略能力を漏洩しうる。'], { legitimacy: 42, compute: 30 }, all('civilization-horizon-observatory', 'governance-compute-registry'), [fx('governance', 2, 'Diplomatic reach +2', '外交到達力+2'), fx('rivalPressure', .06, 'Capability leakage +0.06', '能力漏洩+0.06')]),
  n('civilization', 3, 'recovery-arks', ['Recovery Arks', '文明回復アーク'], ['Preserve diverse knowledge, biospheres, and restart capacity.', '多様な知識・生態系・再起動能力を保存する。'], ['Insurance diverts resources from the race.', '保険が競争から資源を振り向ける。'], { materials: 90, energy: 75, legitimacy: 45 }, all('civilization-branch-embassies', 'resources-closed-loop-mining'), [fx('safety', 3, 'Recovery capacity +3 safety', '回復能力として安全+3'), fx('resources', -2, 'Reserved resources -2', '保存用資源-2')], 'game-inference', { tags: ['industrial-explosion', 'recovery'] }),
  n('civilization', 4, 'solar-system-federation', ['Solar-System Federation', '太陽系連邦'], ['Coordinate habitats and machine ecologies before the Dyson gate opens.', 'Dysonゲート開放前に居住圏と機械生態系を協調させる。'], ['Federation cannot erase light lag or local sovereignty.', '連邦でも光遅延と地域主権は消せない。'], { legitimacy: 120, compute: 110, energy: 85 }, all('civilization-recovery-arks', 'governance-machine-constitution', 'social-abundance-compact'), [fx('governance', 4, 'Federation reach +4', '連邦統治到達力+4'), fx('friction', .1, 'Federal friction +0.1', '連邦摩擦+0.1')]),
  n('civilization', 5, 'interstellar-probes', ['Interstellar Seed Probes', '恒星間シード探査機'], ['Launch slow, corrigible probes after solar expansion is governable.', '太陽系拡張が統治可能になってから低速で修正可能な探査機を放つ。'], ['Once beyond recall, errors become independent histories.', '回収圏外では誤りが独立した歴史になる。'], { compute: 360, energy: 420, materials: 280, legitimacy: 130 }, all('civilization-solar-system-federation', 'resources-dyson-seed', 'safety-corrigible-descendants'), [fx('postDysonExpansion', 6, '+6 civilization', '文明+6'), fx('risk', .22, 'Irreversibility risk +0.22', '不可逆リスク+0.22')], 'game-inference'),
])
